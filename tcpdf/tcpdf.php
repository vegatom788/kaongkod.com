			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (!isset($dom[$key]['attribute']['size']) && !isset($dom[$key]['style']['font-size'])) {
						$headNumber = intval($dom[$key]['value'][1]);
						$headsize = (4 - $headNumber) * 2;

						// Initialize $dom[0]['fontsize'] to avoid division by zero
						$dom[0]['fontsize'] = isset($dom[0]['fontsize']) ? $dom[0]['fontsize'] : 0;

						// Check if $dom[0]['fontsize'] is not zero before operation
						if ($dom[0]['fontsize'] != 0) {
							$dom[$key]['fontsize'] = $dom[0]['fontsize'] + $headsize;
						} else {
							// Handle case where $dom[0]['fontsize'] is zero
							// Perhaps set a default value or handle it appropriately
						}
					}
